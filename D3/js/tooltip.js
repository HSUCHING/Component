...
d3.helper.tooltip = d3.helper.tooltip || function() {
  ...
  function doText(d, i) {
    ...
    tooltipDiv.html(typeof text === "function"
      ? text(d, i)
  ...
  function mm(d, i) {
    ...
    if (arguments.length > 3)
      e = arguments[3];
    var absoluteMousePos = e
      ? [e.pageX, e.pageY]
      : [0, 0];

    tooltipDiv && tooltipDiv.style && tooltipDiv.style('left', (absoluteMousePos[0] > _w / 2
        ? absoluteMousePos[0] - tooltipDiv.node().clientWidth - _p.x
        : absoluteMousePos[0] + _p.x) + 'px').style('top', (absoluteMousePos[1] > _h / 2
  ...
  function tooltip(selection) {
    selection.on("mouseover.tooltip", mover).on('mousemove.tooltip', mm).on("mouseout.tooltip", mout);
  }
